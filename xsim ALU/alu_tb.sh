...
#
# Script generated by Vivado on Thu Sep 18 10:45:53 -0600 2025
...
#
# Usage        : alu_tb.sh
#                alu_tb.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                alu_tb.sh [-reset_run]
#                alu_tb.sh [-reset_log]
#                alu_tb.sh [-help]
#
...
# script info
echo -e "alu_tb.sh - Script generated by export_simulation (Vivado v2025.1 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./alu_tb.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  xelab --incr --debug typical --relax --mt 2  -L xil_defaultlib -L uvm -L unisims_ver -L unimacro_ver -L secureip -L xpm --snapshot alu_tb xil_defaultlib.alu_tb xil_defaultlib.glbl -log elaborate.log
}

# RUN_STEP: <simulate>
simulate()
{
  xsim alu_tb -key {Behavioral:sim_1:Functional:alu_tb} -tclbatch cmd.tcl -log simulate.log
}
...
reset_run()
{
  files_to_remove=(xelab.pb xsim.jou xvhdl.log xvlog.log compile.log elaborate.log simulate.log xelab.log xsim.log run.log xvhdl.pb xvlog.pb alu_tb.wdb xsim.dir libdpi.so)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./alu_tb.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: alu_tb.sh [-help]\n\
Usage: alu_tb.sh [-step]\n\
Usage: alu_tb.sh [-lib_map_path]\n\
Usage: alu_tb.sh [-reset_run]\n\
Usage: alu_tb.sh [-reset_log]\n\
Usage: alu_tb.sh [-keep_index]\n\
Usage: alu_tb.sh [-noclean_files]\n\n\
